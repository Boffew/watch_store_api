...
export class ProductsService {
    ...
    async update(id: number, productDto: UpdateProductDto,file?: Express.Multer.File) {
      ...
      const [existingProduct] = await this.connection.query('SELECT * FROM Products WHERE id = ?', [id]);
      if (!existingProduct) {
        ...
        const result = await this.connection.execute('UPDATE Products SET name = ?, description = ?, image_url = ?, price = ?, brand = ?, color = ?, material = ?, quantity = ?, category_id = ?, updated_at = ? WHERE id = ?', [productDto.name,productDto.description,image_url,productDto.price,productDto.brand,productDto.color,productDto.material,productDto.quantity,productDto.category_id,new Date(),
          id,
        ]);
