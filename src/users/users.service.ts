...
export class UsersService {
    ...
    async createNew(userDto: CreateUserDto){
        ...
        try{
        const result= await this.connection.query('INSERT INTO users (username,password,email,full_name,phone,address,gender,birthday,role,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)', [userDto.username, userDto.password, userDto.email, userDto.full_name, userDto.phone,userDto.address,userDto.gender,userDto.birthday,'Customer',new Date(),new Date()])
